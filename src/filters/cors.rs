...
impl Builder {
    ...
    pub fn allow_origin(self, origin: impl AsRef<str>) -> Self {
    ...
    where
        ...
        I::Item: AsRef<str>,
    {
        let iter = origins.into_iter().map(|s| s.as_ref().to_string());
...
impl Configured {
    ...
    fn is_origin_allowed(&self, origin: &HeaderValue) -> bool {
        // 尝试将 HeaderValue 转换为 &str，如果失败，则返回 false
        let origin_str = match origin.to_str() {
            Ok(s) => s,
            Err(_) => return false,
        };

        // 如果未设置任何允许的 Origin 子字符串，则允许所有 Origin
        if self.cors.origins.is_none() {
            return true;
        }
